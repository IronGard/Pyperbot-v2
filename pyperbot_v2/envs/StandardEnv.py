...
class StandardTestEnv(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        self._done = None
        self._env = None
        self.best_dist_travelled_to_goal = 0
    ...
    #     # if (self.current_step % 100 == 0 and distance_traveled_to_goal > 0):
    #     #     self.reward += 100
    ...
    #     # if (abs(self._client.getEulerFromQuaternion(self._client.getBasePositionAndOrientation(self._snake.get_ids()[0])[1][0])) > 1.5 or
    #     #     abs(self._client.getEulerFromQuaternion(self._client.getBasePositionAndOrientation(self._snake.get_ids()[0])[1][1])) > 1.5 or
    #     #     abs(self._client.getEulerFromQuaternion(self._client.getBasePositionAndOrientation(self._snake.get_ids()[0])[1][2])) > 1.5):
    #     #     self.reward -= 20
    ...
    def step(self, action):
        ...
        if distance_traveled_to_goal > self.max_dist_to_goal:
            self.max_dist_to_goal = distance_traveled_to_goal
        self.reward = distance_traveled_in_step * 1000 + (distance_traveled_in_step*200 if distance_traveled_in_step > 0 else 0)
        self._done = False
        self._truncated = False
        if self._snake.get_base_observation()[0][0] > self.milestone_track:
            self.reward += 100*(self.milestone_track)
            self.milestone_track += 0.1
        if (abs(self._client.getBasePositionAndOrientation(self._snake.get_ids()[0])[0][1]) >  2*(abs(self._client.getBasePositionAndOrientation(self._snake.get_ids()[0])[0][0]))
                and self.current_step > 2000 and self.current_step%10==0):
            self.reward -= (abs(self._client.getBasePositionAndOrientation(self._snake.get_ids()[0])[0][1]*10))
            self.remark = 'heavy movement in y direction'
            self.remark_counter += 1
        #goal reached termination condition
        if dist_to_goal < self._finish_con:
            self.reward += 2000 + (100000/self.current_step)
            self.termination_condition = "goal reached"
            if self.current_step < 10000:
                self.reward += 500
                self.termination_condition = "goal reached early"
        ...
        #early termination condition: if the snake doesn't move at least 0.1 metres closer to the goal within 20% of the episode AND REWARD IS LESS THAN 1e-06
        #i.e. the snake is stuck, terminate the episode. #consider distance limiter as well for the stuck condition - or transform into an intermediate reward.
        elif (self.current_step == 0.4*self.max_episode_steps):
            if distance_traveled_to_goal < 1:
                self._done = True
                self.reward -= 500
                self.termination_condition = "poor performance"
        ...
        if (self.cum_reward < 0) and (self.current_step > 0.5*self.max_episode_steps or self.remark_counter > 3000):
            ...
            self._done = True
        #stop if reward is too low
        if (self.cum_reward < -1000 or distance_traveled_to_goal < -0.2):
            self.termination_condition = "negative reward threshold reached"
            self.reward -= 500
            self._done = True
        #storing actions, rewards and remarks every ten trials
        ...
        if self._done:
            if distance_traveled_to_goal > self.best_dist_travelled_to_goal:
                self.best_dist_travelled_to_goal = distance_traveled_to_goal
                self.reward += 1000
                self.remark = "best distance travelled to goal"
        ...
        if self.num_envs > 1:  # Check if vectorized environments are being used
            ...
            config['DISTANCE'] = {'distance_travelled_to_goal': distance_traveled_to_goal, 'distance_travelled': self.cum_distance, 'distance_to_goal': dist_to_goal,
                                ...
                                'distance_in_timestep': distance_traveled_in_step,
                                'average_speed': self.cum_distance/self.current_step if self.current_step > 0 else 0,
                                'max_dist': self.best_dist_travelled_to_goal}
            config['REWARD'] = {'reward': self.reward, 'cum_reward': self.cum_reward}
            config['GOAL'] = {'goal': self._goal.get_goal_pos()}
            config['SNAKE'] = {'snake_pos': self._client.getBasePositionAndOrientation(self._snake.get_ids()[0])[0], 
                               'snake_ori': self._client.getEulerFromQuaternion(self._client.getBasePositionAndOrientation(self._snake.get_ids()[0])[1]),
